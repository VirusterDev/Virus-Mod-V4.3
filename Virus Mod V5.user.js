...
* Updated mod from KH to SM so you can have soldier instakill
* Added autobullspam
* Worked on autoinsta with bullspam
...
menuHead.innerText = 'Virus Mod V5'
...
<label class="container" style='color:#999'>Auto gg
  <input type="checkbox" id='vagg' checked disabled>
  <span class="checkmark"></span>
</label>
`,'General')
tab.makeTab(`
<label class="container" style='color:#999'>Player tracer
  <input type="checkbox" id='trc' disabled>
  <span class="checkmark"></span>
</label>
<label class="container" style='color:#999'>Activity trace
  <input type="checkbox" id='bTrack' disabled>
  <span class="checkmark"></span>
</label>
`,'Tracers')
tab.makeTab(`
...
tab.makeTab(`<div style="color: #777; height: 100%; width: 90%;">
    <h2>About</h2>
    <p>This version is not even close to being finished, and I'm trying to push it out as quickly as possible.</p>
    <p>This version is for SM (Sword + Musket). The next version will probably be for everyone since it will be released when it's finished, not at a waypoint of subscribers.</p>
    <p>Since this mod is for SM, your best offensive/attack strategy is to take advantage of the game's clown counting system.</p>
    <p>The MooMoo.io clown counting system:</p>
    <ul>
        <li>If you heal under 120 milliseconds after being damaged, you will receive one clown counting point. If this goes above 7, you will get the clown hat and not be able to heal.</li>
        <li>If you heal more than 120 milliseconds after being damaged, your clown counter will be subtracted by two.</li>
        <li>An InstaKill is faster than 120 milliseconds.</li>
    </ul>
    <p>This mod has autobullspam + smart autoinsta, which will automatically hit the enemy repeatedly. Then the enemy has two options: get clown hat or die from instakill.</p>
    <p>Think about the instakill as two parts (Bull hit, then musket + turret).</p>
    <p>So when the bullspam triggers, the enemy's antiinsta may toggle to prevent the part two of the instakill, but it will not fire because the mod will detect if the enemy has healed.</p>
    <p>Once again, the mod autobullspams, then if the enemy's antiinsta toggles, it will not instakill, but if it doesn't, the enemy will be killed.</p>
    <p>However, you will need to watch out for the players that have autoemp. Autoemp will automatically equip the emp helmet after being hit, making the instakill not do enough damage.</p>
    <p>If this is the enemy, then I recommend just using your brain and strategy.</p>
    <p>A lot of the actual moomoo.io code has been recoded for performance.</p>
    <p>Don't use the antipingpong spike since it will cause lag.</p>
</div>`,'Information')
       document.querySelector('#render').addEventListener('change',(e) => {
           virus.renderDistance = e.path[0].value
           console.log(e.path[0].value)
       })
       document.querySelector("#atgrind").addEventListener('change', function() {
           virus.togAutoGrind()
       });
...
if(virus.WR_Insta && nearestEnemy.length !== 0 && _.sid == nearestEnemy[0]){
    be.drawImage(targetImg2, (_.x - f) - 50, (_.y - d) - 50,100,100);
           ...
           if(nearestEnemy.length){
           let oio = Ii(nearestEnemy[0])
           if(R.primary.reload == 1 && dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5){
               GameLog.log('Bullspamming')
           Jt(ID_BullHelmet, !1);
           Jt(0, !0);
           r.send("5", R.weapons[0], true);
           r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
           r.send("c", 0, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
               if(R.secondary.reload == 1){
                   let clownCounter = oio.shameCounter;
                   setTimeout(() => {
                       let updated = Ii(nearestEnemy[0])
                       if(clownCounter == updated.shameCounter && updated.health !== 100){
                           GameLog.log('Heal Detect, insta time')
                           virus.status = 'Autoinsta'
                           Jt(ID_TurretGear, !1)
           ...
           */
           ...
           if(virus.status == 'Await' && !virus.autogrind){
               if(true){
                   if(R.secondary.reload == 1 && R.weaponIndex !== R.weapons[0]){
                       r.send("5", R.weapons[0], true);
                       console.log('autoreload to primary', R.secondary.reload)
                   }else if(R.secondary.reload !== 1){
                       r.send("5", R.weapons[1], true);
                       console.log('autoreload to secondary', R.secondary.reload)
                   }
                   /*
               ...
               if(virus.status == 'Insta1'){
                   virus.status = 'Insta2'
                   //console.log(virus.status)
               }else if(virus.status == 'Insta2' || virus.status == 'Spiketick' || virus.status == 'Bullspam'){
