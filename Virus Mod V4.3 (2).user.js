...
* Auto hat switch
* Added Auto push
* Added One frame hatswitch
* Broke the instakill
* Switched from KH mod to PH mod
* Added teleport hotkey because no one fucking plays this game, so I have to go find them
* Added anti turret (autoemp) because everyone wants to fucking camp with turrets and insta
* Added antilag (babled code)
* Switched mod back to KH
* Added bullspam
* Updated autoplacer when bullspam is activated
* Added one frame hatswitch for bullspam

**I'm legit not going to stop until I cannot be killed.
**I cannot be instakilled if my ping is under 100 :skull:

**Uh so yeah, I'm releasing this mod because I'm switching the mod from KH to SM (Sword + Musket). So I'm going to release the KH version just bc
*/
window.cpmstarAPI = () => { };
...
doc.write(content);
doc.close();
[...doc.getElementsByTagName("script")].find(e => e?.src.endsWith("bundle.js") || e?.src.includes('frvr') || e?.src.includes('google'))?.remove();
...
function recordCanvas(canvas) {
  ...
  const recorder = new MediaRecorder(stream, { mimeType: 'video/webm; codecs=vp9', audioBitsPerSecond: 0, videoBitsPerSecond: 10});
  recorder.ondataavailable = (event) => {
      if(chunks.length >= 0)chunks.shift();
...
recordCanvas(document.querySelectorAll('canvas')[1])
/*
setTimeout(() => {
...
if(remover || remover2 || remover3 || remover4||remover7){
    remover.remove()
    remover2.remove()
    remover3.remove()
    remover4.remove()
...
const targetImg2 = new Image()
targetImg2.src = 'https://snipboard.io/8pvWQk.jpg';
// --- Hats ---
var ID_Shame = 45
, ID_MooCap = 51
, ID_AppleCap = 50
...
, ID_Blocker = 37
, ID_Teleporter = 38
virus.hats.push({id: ID_MonkeyTail,cost: 2000, hat: false})
virus.hats.push({id: ID_SoldierHelmet,cost: 4000, hat: true})
virus.hats.push({id: ID_BullHelmet,cost: 6000, hat: true})
virus.hats.push({id: ID_EmpHelmet,cost: 7000, hat: true}) // ik its wrong price, its bc the bull hat and the emp are the same price, and the emp doesnt get bought
virus.hats.push({id: ID_TurretGear,cost: 10000, hat: true})
virus.hats.push({id: ID_SpikeGear,cost: 11000, hat: true}) //same w/ this
virus.hats.push({id: ID_TankGear,cost: 15000, hat: true})
virus.hats.push({id: ID_CorruptXWings,cost: 20000, hat: false})
var tick = 0;
!function(e) {
    var t = {};
    function n(i) {
 ...
 */
   ...
   , function(e, t, n) {
       (function(t) {
           e.exports.maxScreenWidth = 1920,
               ...
               e.exports.weaponVariants = window.weaponVariants = [{
                   id: 0,
                   src: "",
                   xp: 0,
                   val: 1
   ...
   , function(e, t, n) {
    ...
    document.getElementById("pingDisplay")), Ee = document.getElementById("shutdownDisplay"), Me = document.getElementById("menuCardHolder"), Ae = document.getElementById("guideCard"), Pe = document.getElementById("loadingText"), Be = document.getElementById("gameUI"), Ce = document.getElementById("actionBar"), Oe = document.getElementById("scoreDisplay"), Re = document.getElementById("foodDisplay"), je = document.getElementById("woodDisplay"), _e = document.getElementById("stoneDisplay"), Ue = document.getElementById("killCounter"), De = document.getElementById("leaderboardData"), Le = document.getElementById("nameInput"), Fe = document.getElementById("itemInfoHolder"), ze = document.getElementById("ageText"), He = document.getElementById("ageBarBody"), Ve = document.getElementById("upgradeHolder"), qe = document.getElementById("upgradeCounter"), Ye = document.getElementById("allianceMenu"), We = document.getElementById("allianceHolder"), Xe = document.getElementById("allianceManager"), Ne = document.getElementById("mapDisplay"), Ge = document.getElementById("diedText"), Je = document.getElementById("skinColorHolder"), Ke = Ne.getContext("2d");
       ...
       var Vt, qt = [];
       function Yt(e, t) {
           if(virus.sync && R.primary.reload == 1 && dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5){
                       virus.status = 'TeamSync'
                       Jt(ID_BullHelmet, !1)
                       Jt(0, !0)
                       r.send("5", R.weapons[0], true);
                       r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                       r.send("c", 0, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                   }
           ...
           27 == t ? Qt() : R && R.alive && kn() && (gn[t] || (gn[t] = 1,
                                                               69 == t ? r.send("7", 1) : 67 == t ? (Mt || (Mt = {}),
                                                                                                     ...
                                                                                                     Mt.y = R.y) : 88 == t ? (R.lockDir = R.lockDir ? 0 : 1,
                                                                                                                              r.send("7", 0)) : null != R.weapons[t - 49] ? Sn(R.weapons[t - 49], !0) : null != R.items[t - 49 - R.weapons.length] ? Sn(R.items[t - 49 - R.weapons.length]) : 81 == t ? Sn(R.items[0]) : 82 == t ? xn() : mn[t] ? bn() : 999 == t && (O = 1,
        vn())))
       }
                                                         ))),
               ...
               Me.style.display = "block",
                   ...
                   Ge.style.display = "none"
                           ...
                           e && t ? (wt = Date.now(),
                                     xt()) : Tn();
               let interval = setInterval(() => {
                   r.send('33', Math.atan2((It.y - R.y2), (It.x - R.x2)))
               },1000)
               setTimeout(() => {
                   clearInterval(interval)
               },10000)
       ...
       function Cn(e) {
           ...
           if(virus.autogrind){
               cplace(R.items[5], 0)
               cplace(R.items[5], virus.math.toRad(90))
               cplace(R.items[5], virus.math.toRad(-90))
               cplace(R.items[5], virus.math.toRad(180))
           }
               let buildingthing = Mi(e);
               //virus mod v2 spike tick
               /*Sn(R.items[2]);
               r.send('c', 1, _+Math.PI/3);
               r.send('c', 0, _+Math.PI/3);
               Sn(R.primary, 1);
               setTimeout(() => {
                   Jt(7, 21)
               },0)*/
           if(Math.hypot(buildingthing.y - R.y2, buildingthing.x - R.x2) > 500){virus.rangeL =
               {
               y: buildingthing.y,
               x: buildingthing.x
               }}
               if(allEnemy.length !== 0 && buildingthing.group.name == 'trap' && R.primary.reload == 1 && dist(nearestEnemy, R) <= 135 && (buildingthing.owner.sid == R.sid || window.isAlly(buildingthing.owner.sid))){
           virus.status = 'Spiketick'
                   Sn(R.items[2]);
                   r.send("2", Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                   Jt(ID_BullHelmet, !1)
                   Jt(0, !0)
                   r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                   r.send("c", 0, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                   ...
                   if(virus.status == 'Autobreak' && buildingthing.group.name == 'trap' && dist(nearestEnemy, R) <= 150 && (buildingthing.owner.sid !== R.sid && !window.isAlly(buildingthing.owner.sid))){
                       virus.status = 'Antiinsta'
                       Jt(ID_SoldierHelmet, !1)
                       cplace(R.items[2], Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)))
                   /*for(var spamplace=0; spamplace < 360; spamplace += 10){
                   cplace(R.items[2], pn() + virus.math.toRad(spamplace))
                   }*/
                   }
               }
       }
       ...
       function Dn(e, t, n) {
           null != e && (R.XP = e),
               null != t && (R.maxXP = t),
               null != n && (R.age = n),
               n == o.maxAge ? (ze.innerHTML = "bro placed too many mills",
       ...
       function Yn(e, t, n) {
           if (R) {
    for (var i, r, s, a = 0, len = N.length; a < len; ++a) {
        var node = N[a];
        if (!node.globalAlpha) {
            node.globalAlpha = 0;
        }
        var hypot = Math.hypot(node.y - R.y2, node.x - R.x2);
        if (R && hypot < virus.renderDistance) {
            if (node.globalAlpha !== 1 && virus.renderfade) {
                node.globalAlpha += 0.01;
                if (node.globalAlpha > 1) {
                    node.globalAlpha = 1;
                }
            }
            if (node.active) {
                r = node.x + node.xWiggle - t;
                s = node.y + node.yWiggle - n;
                if (e == 0) {
                    node.update(P);
                }
                if (node.layer == e && ki(r, s, node.scale + (node.blocker || 0))) {
                    if(virus.renderfade)be.globalAlpha = node.globalAlpha;
                    i = node.isItem ? ri(node) : ni(node);
                    if (node.isItem) {
                        be.save();
                        be.translate(r, s);
                        //be.rotate(node.dir);
                        ...
                        if (node.blocker) {
                            ...
                            be.lineWidth = 6;
                            si(0, 0, node.blocker, be, false, true);
                        }
                        be.restore();
                    } else {
        ...
        } else if (R && hypot > virus.renderDistance) {
            if (virus.renderfade && node.globalAlpha !== 0) {
                node.globalAlpha -= 0.01;
                if (node.globalAlpha < 0) {
                    node.globalAlpha = 0;
                }
            }
            if (node.active && node.globalAlpha > 0) {
                r = node.x + node.xWiggle - t;
                s = node.y + node.yWiggle - n;
                if (e == 0) {
                    node.update(P);
           ...
           for (var i, r, s, a = 0; a < N.length; ++a){
               ...
               if(R && Math.hypot(N[a].y-R.y2, N[a].x-R.x2) < virus.renderDistance){
                   if(N[a].globalAlpha !== 1)N[a].globalAlpha += 0.1;
               (_ = N[a]).active && (r = _.x + _.xWiggle - t,
                                     s = _.y + _.yWiggle - n,
                                     0 == e && _.update(P),
                                     _.layer == e && ki(r, s, _.scale + (_.blocker || 0)) && (be.globalAlpha = N[a].globalAlpha ? .6 : 1,
                                                                                              _.isItem ? (i = ri(_),
                                                                                                          be.save(),
                                                                                                          be.translate(r, s),
                                                                                                          //be.rotate(_.dir),
                                                                                                          be.globalAlpha = N[a].globalAlpha,
                                                                                                          be.drawImage(i, -i.width / 2, -i.height / 2),
                                                                                                          ...
                                                                                                          be.restore()) : (i = ni(_),
                                                                                                                           be.drawImage(i, r - i.width / 2, s - i.height / 2))));
           }else if(R && Math.hypot(N[a].y-R.y2, N[a].x-R.x2) > virus.renderDistance){
               if(N[a].globalAlpha !== 0)N[a].globalAlpha -= 0.1;
               if(N[a].globalAlpha > 0){
               (_ = N[a]).active && (r = _.x + _.xWiggle - t,
                                     s = _.y + _.yWiggle - n,
                                     0 == e && _.update(P),
                                     _.layer == e && ki(r, s, _.scale + (_.blocker || 0)) && (be.globalAlpha = N[a].globalAlpha ? .6 : 1,
       ...
       function Wn(e, t, n) {
           ...
           _ = Ii(e)
           if(_.sid == R.sid && virus.status == 'Autobreak' && R.skins[ID_SoldierHelmet])Jt(ID_SoldierHelmet, !1)
           if(virus.status == 'Antibull'){
               virus.status = 'AutoHit'
               GameLog.log(virus.status, '#777')
                   Jt(ID_BullHelmet, !1)
                   Jt(0, !0)
                   r.send("5", R.weapons[0], true);
                   r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
           ...
           if(_.secondary.id !== null && _.secondary.reload == 1 && !window.isAlly(_.sid)){
               if(_.weaponIndex == ID_GreatHammer && _.primary.reload == 1 && _.skinIndex == ID_TurretGear){
                   if(R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet)Jt(ID_SoldierHelmet, !1)
                   if(R.shameCount <= 4 && R.health !== 100)virus.heal(100 - R.health);
               }
               if(_.secondary.id == ID_Crossbow && _.primary.id == ID_Polearm){
                   if(R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet)Jt(ID_SoldierHelmet, !1)
                   virus.status = 'Antiinsta'
                   if(R.shameCount <= 4 && R.health !== 100)virus.heal(100 - R.health);
               }
               if(_.secondary.id == ID_Musket && _.primary.id == ID_Polearm){
                   if(R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet)Jt(ID_SoldierHelmet, !1)
                   //virus.status = 'Antiinsta'
                   if(R.shameCount <= 6 && R.health !== 100)virus.heal(100 - R.health);
               }
               if(_.secondary.id == ID_Musket && _.primary.id == ID_ShortSword && (_.weaponVariant == 2 || _.weaponVariant == 3)){
                   if(R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet)Jt(ID_SoldierHelmet, !1)
                   virus.status = 'Antiinsta'
                   if(R.shameCount <= 4 && R.health !== 100)virus.heal(100 - R.health);
               }
               if(_.secondary.id == ID_Musket && _.primary.id == ID_ShortSword && (_.weaponVariant == 0 || _.weaponVariant == 1) && R.skinIndex !== ID_SoldierHelmet){
                   if(R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet)Jt(ID_SoldierHelmet, !1)
                   virus.status = 'Antiinsta'
                   if(R.shameCount <= 6 && R.health !== 100)virus.heal(100 - R.health);
               }
    ...
    if (_) {
        _.startAnim(t, n);
        if (_.sid == R.sid && virus.status == 'Autobreak' && R.skins[ID_SoldierHelmet]) {
            Jt(ID_SoldierHelmet, false);
        }
        if (virus.status == 'Antibull') {
            virus.status = 'AutoHit';
            GameLog.log(virus.status, '#777');
            Jt(ID_BullHelmet, false);
            Jt(0, true);
            r.send('5', R.weapons[0], true);
            r.send('c', 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
            r.send('c', 0, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
        ...
        if (_.secondary.id !== null && _.secondary.reload == 1 && !window.isAlly(_.sid)) {
            if (_.weaponIndex == ID_GreatHammer && _.primary.reload == 1 && _.skinIndex == ID_TurretGear) {
                if (R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet) {
                    Jt(ID_SoldierHelmet, false);
                }
                if (R.shameCount <= 4 && R.health !== 100) {
                    virus.heal(100 - R.health);
                }
            } else if (_.secondary.id == ID_Crossbow && _.primary.id == ID_Polearm) {
                if (R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet) {
                    Jt(ID_SoldierHelmet, false);
                }
                virus.status = 'Antiinsta';
            ...
            } else if (_.secondary.id == ID_Musket && _.primary.id == ID_Polearm) {
                if (R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet) {
                    Jt(ID_SoldierHelmet, false);
                }
                virus.heal(100 - R.health);
            } else if (_.secondary.id == ID_Musket && _.primary.id == ID_ShortSword) {
                if (_.weaponVariant == 2 || _.weaponVariant == 3) {
                    if(_.skinIndex == ID_BullHelmet && R.skinIndex == ID_SoldierHelmet && R.skins[ID_EmpHelmet]){
                        virus.autoEmp = true
                        Jt(ID_EmpHelmet, false);
                        console.log('autoemp')
                        setTimeout(() => {
                            virus.autoEmp = false
                        },111)
                    }else if(_.skinIndex == ID_BullHelmet && R.skinIndex !== ID_SoldierHelmet){
                        if(R.shameCounter < 7)virus.heal(100 - R.health);
                        if(!R.skins[ID_EmpHelmet])sn('chill i got no hats bruh')
                    }
                    /*
                    if (R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet && _.skinIndex == ID_BullHelmet) {
                        Jt(ID_SoldierHelmet, false);
                        virus.heal(100 - R.health);
                    }else{
                        if(R.skins[ID_EmpHelmet]){
                            Jt(ID_SoldierHelmet, false);
                        }else{
                        }
                    }*/
                } else if ((_.weaponVariant == 0 || _.weaponVariant == 1) && R.skinIndex !== ID_SoldierHelmet) {
                    if (R.skins[ID_SoldierHelmet] && R.skinIndex !== ID_SoldierHelmet) {
                        Jt(ID_SoldierHelmet, false);
                    }
                    virus.status = 'Antiinsta';
                    if (R.shameCount <= 6 && R.health !== 100) {
                        virus.heal(100 - R.health);
                    }
                }
            }
       ...
       function ri(e, t) {
           ...
           if (!n || t) {
               ...
               if (r.translate(i.width / 2, i.height / 2),
                   ...
                   r.fillStyle = "poison spikes" == e.name ? "#000" : "#000";
                   var d = .6 * e.scale;
                   ai(r, "spikes" == e.name ? 5 : 6, e.scale, d),
                       ...
                       r.fillStyle = "#a5974c",
                       si(0, 0, d, r),*/
                       r["fillStyle"]=(e["isItem"]?(e["owner"]["sid"]==R["sid"]||window["isAlly"](e["owner"]["sid"])?"#c9b758":"#ff2222"):"#a5974c"),//lol i just felt like making it look weird
                       si(0, 0, d / 2, r, !0)
       ...
       function fi(e, t, n, i, r, s, a, o) {
           if(lt) {
               virus.status = 'Antiinsta'; if(R.health <= 50)virus.heal(50)
               ...
               for(let ii = 0; ii < W.length; ii++) {
                   let _ = W[ii];
                   if(_.visible) {
                       if(r == 1.5 && Math.hypot(_.y2 - t, _.x2 - e) < 35 && _.skinIndex == 53) {
                           _.turret = -0.0444;
                       }else if(_.weaponIndex > 9 && (l.weapons[_.secondary.id].projectile !== undefined || l.weapons[_.secondary.id].projectile !== null) && Math.hypot(_.y2 - bulletPos.y, _.x2 - bulletPos.x) < (R == _ ? 70 : 35)) {
                           setTimeout(() => {
                               _.secondary.reload = -111/l.weapons[_.secondary.id].speed;
                               if(_.sid == R.sid && _.secondary.reload >= 0.9 && virus.inTrap){
                               R.primary.reload = 1
                               }
                           });
                       }
       ...
       document.addEventListener('keydown', (e) => {
           ...
           if(nearestEnemy.length){
               console.log('loading up autopush')
               let myTraps = [], mySpikes = []
               let nET, nTS;
               for(var ggg = 0; ggg < N.length; ggg++){
                   let build = N[ggg]
                   if(build.trap && build.active && build.owner.sid == R.sid){
                       if(Math.hypot(build.y-R.y2, build.x-R.x2) <= 500){
                           myTraps.push(build)
                           console.log('mynewtraps', myTraps)
                       }
                   }
                   try{
                       if(build.group.name == 'spikes' && build.active && build.owner.sid == R.sid){
                           if(Math.hypot(build.y-R.y2, build.x-R.x2) <= 500){
                               mySpikes.push(build)
                               console.log('mynewspike', mySpikes)
                           }
                       }
                   }catch(e){console.log('error', e)}
               }
               if(myTraps.length && mySpikes.length){
                   myTraps = myTraps.sort((a,b) => Math.hypot(nearestEnemy[2]-a.y, nearestEnemy[1]-a.x) - Math.hypot(nearestEnemy[2]-b.y, nearestEnemy[1]-b.x));
                   nET = myTraps[0];
                   console.log('nearestenemytrap', nET)
                   mySpikes = mySpikes.sort((a,b) => Math.hypot(nET.y-a.y, nET.x-a.x) - Math.hypot(nET.y-b.y, nET.x-b.x));
                   nTS = mySpikes[0];
                   console.log('nearestTrapSpike', nTS)
                   if(Math.hypot(nET.y-nTS.y, nET.x-nTS.x) <= 150){
                       console.log('heres where the spike tick begins lol')
                       const distanceBehindEnemy = 50;
                       const angleToSpike = Math.atan2(nTS.y - nearestEnemy[2], nTS.x - nearestEnemy[1]);
                       const newX = nearestEnemy[1] + (distanceBehindEnemy * Math.cos(angleToSpike + Math.PI));
                       const newY = nearestEnemy[2] + (distanceBehindEnemy * Math.sin(angleToSpike + Math.PI));
                       let clearr = true
                       let moveinterval = setInterval(() => {
                           if(Math.hypot(newY-R.y, newX-R.x) > 10){
                       r.send('33', Math.atan2((newY - R.y2), (newX - R.x2)))
                           }else{
                               clearInterval(moveinterval)
                               clearr = false
                               let lolinterval = setInterval(() => {
                               r.send('33', Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)))
                               },100)
                               setTimeout(() => {clearInterval(lolinterval)},3000)
                           }
                       },100)
                       setTimeout(() => {
                           if(clearr){
                   ...
                   case 'f':
                       ...
                       break;
                   case 'q':
                       virus.qholdisnoob = true
                       break;
                   case '1':
                       virus.WRInstaSeekDelay = parseInt(prompt('#virus'))
                       break;
                   case 'n':
                       virus.mills = !virus.mills
                       break
               }
           }
       })
           ...
           if(document.activeElement.id.toLowerCase() !== 'chatbox'){
               switch(e.key){
                   ...
                   case 'f':
                       ...
                       break;
                   case 'q':
                       virus.qholdisnoob = false
                       break;
                   case ' ':
                       virus.tickMode = false
                       break;
               }
           }
       })
       setInterval(() => {
           r.send("2", pn())
           let gold = parseInt(document.querySelector('#scoreDisplay').innerText)
           for(var i=0; i < virus.hats.length; i++){
               if(virus.hats[i].cost){
                   if(gold >= virus.hats[i].cost){
                       if(virus.hats[i].hat == true){
                           Kt(virus.hats[i].id, !1);
                       }else{
                           Kt(virus.hats[i].id, !0);
                       }
                       virus.hats[i] = {}
                       //console.log(virus.hats[i])
                   }
               }
           }
           if(virus.toxic){
               let ar = ["Don't care", "didn't ask", "cry about it", "stay mad", "get real", "L", "mald seethe cope harder", "hoes mad", "basic", "skill issue", "ratio", "you fell off", "the audacity", "triggered", "any askers", "replied", "get a life", "ok and?", "cringe", "touch grass", "donwalled", "not based", "not funny didn't laugh", "*you're", "grammar issues", "go outside", "get good", "reported", "ad hominem", "GG!", "ask deez", "ez clap", "straight cash", "ratio again", "final ratio", "problematic", "furry lover", "men lover", "script kiddie"]
               r.send('ch', ar[Math.floor(Math.random() * ar.length)])
           }
       },500)
       /*
           const currentZoom = parseFloat(window.getComputedStyle(document.body).zoom);
           const newZoom = currentZoom / 2;
           document.body.style.zoom = newZoom;*/
       setInterval(() => {
           /*document.querySelector('#gameCanvas').style.width = '100%';
           document.querySelector('#gameCanvas').style.height = '100%';*/
           //console.log('mills try trigger')
           if(virus.mills == true && R.itemCounts[3] !== 99 && virus.status !== 'Autobreak'){
               //console.log('mills trigger')
               cplace(R.items[3], (virus.lastMoveDir + virus.math.toRad(180)))
               cplace(R.items[3], (virus.lastMoveDir + virus.math.toRad(180) + virus.math.toRad(77.7)))
               cplace(R.items[3], (virus.lastMoveDir + virus.math.toRad(180) - virus.math.toRad(77.7)))
           }else if(virus.mills){
               virus.mills = false
           }
...
function buttonUnPressed(buttonIndex) {
    console.log(buttonIndex)
    switch(buttonIndex){
...
let step = ()=>{
	if(gamepadIndex !== undefined) {
        ...
        utils.gamepad = myGamepad;
		myGamepad.buttons.map(e => e.pressed).forEach((isPressed, buttonIndex) => {
            if(utils.buttons[buttonIndex]){
			if(isPressed) {
                if(utils.buttons[buttonIndex].pressed == false){
                    buttonPressed(buttonIndex)
                }
                utils.buttons[buttonIndex].pressed = true

			}else{
                if(utils.buttons[buttonIndex]){
                    if(utils.buttons[buttonIndex].pressed == true){
                        buttonUnPressed(buttonIndex)
                    }
                    utils.buttons[buttonIndex].pressed = false
                }else{
                    utils.buttons[buttonIndex] = {}
                }

            }
        }else{
            utils.buttons[buttonIndex] = {}
            utils.buttons[buttonIndex].pressed = isPressed
        }
		})
  let a = 0;
  let b = 0;
  if (utils.gamepad.axes[0] !== 0) {
    b -= utils.gamepad.axes[0];
  } else if (utils.gamepad.axes[1] !== 0) {
    a += utils.gamepad.axes[1];
  } else if (utils.gamepad.axes[2] !== 0) {
    b += utils.gamepad.axes[2];
  } else if (utils.gamepad.axes[3] !== 0) {
    a -= utils.gamepad.axes[3];
  }
        console.log(utils.gamepad.axes[0] + utils.gamepad.axes[1])
	}
...
requestAnimationFrame(step)
    ...
    appendElement:(element, where) => {
        where.appendChild(element)
    },
    makeStyleSheet:(rules) => {
        var stylesheet = Virus.createElement('style')
        stylesheet.innerHTML = rules
        Virus.appendElement(stylesheet,top.document.body)
    }
}
       let L1 = Virus.createElement('div')
       let L2 = Virus.createElement('div')
       L1.appendChild(L2)
       L1.style=`
  height: 100%;
  width: 100%;
  ...
  margin: -50px 0 0 -50px;
       `
       top.document.body.appendChild(L1)
var menu = Virus.createElement('div')
menu.id = 'VirusCheatMenu'
Virus.appendElement(menu,top.document.body)
...
var menutop = Virus.createElement('div')
menutop.id = 'VirusTop'
Virus.appendElement(menutop,menu)
Virus.makeStyleSheet(`
#VirusCheatMenu{
width:500px;
...
GameLog = {
    log: (txt, clr) => {
        var element = Virus.createElement('p')
        element.className = 'VirusGameLogLogs'
        element.style.color = clr
        element.innerHTML = txt;
        Virus.appendElement(element, menu.querySelector('#GameLogTabLogger'))
    }
}

               virus.projHandle = ()=>{if(R.health !== 100 && R.shameCount <= 5)virus.heal(100-R.health)}
               virus.reloadHandle = (e) => {
                   if(R.primary.reload == 1 && !window.isAlly(e.sid) && R.sid !== e.sid && R.skins[ID_SpikeGear] && R.tails[ID_CorruptXWings] && (e.primary.id == ID_Katana || e.primary.id == ID_Polearm)){
                       if(dist(nearestEnemy, R) <= l.weapons[e.primary.id].range * 1.5){
                       virus.status = 'Antibull'
               GameLog.log(virus.status, '#777')
                   Jt(ID_SpikeGear, !1)
                   Jt(ID_CorruptXWings, !0)
                       }
                   }
               }
/*
    GM_registerMenuCommand('Crash server', ()=>{
    if(confirm('your gay if you do dis, u sureee?')){
        r.close()
    }
    })*/
var menutabs = Virus.createElement('div')
menutabs.id = 'VirusTabDiv'
Virus.appendElement(menutabs,menu)
var menuHead = Virus.createElement('p')
menuHead.id = 'VirusHead'
menuHead.innerText = 'Virus Mod V4.3 KH version'
Virus.appendElement(menuHead, menutop)
var tab = {
    makeTab: (content, txt) => {
        var tab777 = Virus.createElement('button')
        tab777.className = 'VirusTabButton';
        tab777.innerText = txt;
        Virus.appendElement(tab777, menutabs);
        var tabdiv = Virus.createElement('div')
        tabdiv.className = 'VirusTabDivs'
        tabdiv.innerHTML = content;
        tabdiv.style.display = 'none'
        if(txt == 'Game Log'){
            tabdiv.style.display = 'block'
        }
        Virus.appendElement(tabdiv,menu)
        tab777.onclick = function(){
            let tabz = menu.querySelectorAll('.VirusTabButton')
 for(var i = 0; i < tabz.length; i++){
    tabz[i].style.backgroundColor = 'transparent'
}
let tabzz = menu.querySelectorAll('.VirusTabDivs')
 for(let i = 0; i < tabzz.length; i++){
    tabzz[i].style.display = 'none'
    /*tabzz[i].onHover = function(){
        tabzz[i].style.backgroundColor = '#112'
    }*/
}
tabdiv.style.display = 'block'
tab777.style.backgroundColor = '#000'
...
<label class="container" style='color:#999'>Anti ping pong spike
  <input type="checkbox" id='pingpong'>
  <span class="checkmark"></span>
</label>
<label class="container" style='color:#999'>Team sync
  <input type="checkbox" id='sync'>
  <span class="checkmark"></span>
</label>

<label class="container" style='color:#999'>Auto hit
  <input type="checkbox" id='hit'>
  <span class="checkmark"></span>
</label>

<label class="container" style='color:#999'>No look
  <input type="checkbox" id='spin'>
  <span class="checkmark"></span>
</label>

<label class="container" style='color:#999'>Toxic
  <input type="checkbox" id='noob'>
  <span class="checkmark"></span>
</label>
<label class="container" style='color:#999'>Auto gg
  <input type="checkbox" id='vagg'>
  <span class="checkmark"></span>
</label>
`,'General')
tab.makeTab(`
<label class="container" style='color:#999'>Player tracer
  <input type="checkbox" id='trc'>
  <span class="checkmark"></span>
</label>
<label class="container" style='color:#999'>Activity trace
  <input type="checkbox" id='bTrack'>
  <span class="checkmark"></span>
</label>
`,'Tracers')
tab.makeTab(`

<label class="container" style='color:#999'>Auto break
  <input type="checkbox" id='autobreaker'>
  <span class="checkmark"></span>
</label>
<label class="container" style='color:#999'>Auto buy
  <input type="checkbox" id='buyer'>
  <span class="checkmark"></span>
</label>
<label class="container" style='color:#999'>Auto grind
  <input type="checkbox" id='atgrind'>
  <span class="checkmark"></span>
</label>
`,'Automation')
       tab.makeTab(`
<label class="container" style='color:#999'>Auto place
  <input type="checkbox" id='autoplacer'>
  <span class="checkmark"></span>
</label>

<label class="container" style='color:#999'>Auto replace
  <input type="checkbox" id='autoreplacer'>
  <span class="checkmark"></span>
</label>
`,'Build')
       tab.makeTab(`<div id='GameLogTabLogger'></div>`,'Game Log')
       tab.makeTab(`
       `,'Instakill')
       tab.makeTab(`
       <style>
...
input[type=range]:focus::-ms-fill-upper {
  background: #555;
       ...
       <p>Render Distance<input id='render' type="range" min=0 max=1250></p>
<label class="container" style='color:#999'>Render fade effects
  <input type="checkbox" id='renderfade'>
  <span class="checkmark"></span>
</label>
       `,'Visuals')
tab.makeTab(`<div style="color: #777; height: 100%; width: 90%;">
  <h2>About</h2>
  <p>
    If you randomly reload, it means you were disconnected due to packet spamming. This mod is designed for KH (Katana + Hammer), but this will change in the next update.
  </p>
  <p>
    Below, I'll explain how to use it and provide some tips for using KH.
  </p>

  <h2>How to Use</h2>
  <ul>
    <li>Do not stop moving when using windmills.</li>
    <li>The mod has an autoplacer, which means you don't have to do anything to build, but you still have the option to.</li>
    <li>The mod also has Spike Tick, which is essential when using KH. Spike Tick will automatically replace a broken trap with a spike and hit at the same tick.</li>
    <li>Spike Tick is a one-tick/frame instakill. To use it, trap the enemy and stand next to them, make sure the mod can place a spike in the spot where the trap is, and remember to be as close as possible.</li>
    <li>The mod also has Bullspam, Autohit, Spikesync, and Autopush.</li>
    <li>Bullspam repeatedly hits the enemy, making them vulnerable and unstable. You can use Bullspam by holding down the space key and being in weapon range of the enemy.</li>
    <li>Autohit automatically hits the enemy when their health is low enough to kill them.</li>
    <li>Spikesync may be disrupted by Autohit but will still work. Spikesync will hit the enemy when they hit your spike. If they don't have a soldier helmet, they will die.</li>
    <li>Autopush is not that tactical but is good for killing legit players and showing off. Pushing is when the enemy is in the trap, and you push them into a spike. When they're in the trap, they won't take any knockback, even from spikes. So if they're being hit by a spike while in the trap, they won't move and will be hit rapidly in each game update tick.</li>
    <li>You can use Autopush by placing a spike as close as possible to a trap, then press Z, and it will automatically move your player to the enemy's "shadow." Think of the spike as a light source.</li>
    <li>Clicking the game canvas will toggle breaking mode, which is like Autobreak in traps, but not in the direction of the trap. You can use this to break enemy spikes, clear out areas for 1v1s, and more.</li>
  </ul>

  <p>
    The mod is built for lightweight gameplay, and a lot of the moomoo.io game code has been recoded for performance. Here is a list of hotkeys:
  </p>

  <ul>
    <li>[V] Spike</li>
    <li>[F] Trap</li>
    <li>[Space] Bullspam</li>
    <li>[Esc] Menu toggle</li>
  </ul>
</div>
`,'Information')
       document.querySelector('#render').addEventListener('change',(e) => {
           virus.renderDistance = e.path[0].value
           console.log(e.path[0].value)
       })
       var pingpong = document.querySelector("#pingpong")
       pingpong.addEventListener('change', function() {
           virus.antipingpong = this.checked
       });

       var renderfade = document.querySelector("#renderfade")
       renderfade.addEventListener('change', function() {
           virus.renderfade = this.checked
       });
       var syncy = document.querySelector("#sync")
       syncy.addEventListener('change', function() {
           virus.sync = this.checked
       });
       var mi = {};
       var Acc = [11, 13, 21, 18];
       var Hat = [6, 7, 12, 15, 22, 31, 40, 53, 11];
       function yi(e, t) {
           var n = e.index
           , i = mi[n];
           if (!i) {
       ...
       window.secondaryDmg = function(id) {
           if(!window.weapons[id].projectile) return 0;
           if(!window.projectiles[window.weapons[id].projectile].dmg) return 0;
           return window.projectiles[window.weapons[id].projectile].dmg;
       };
       window.variantMulti = function(id) {
           return window.weaponVariants[id].val;
       };
       ...
       window.place = place;
       virus.togAutoGrind = () => {
           virus.autogrind = !virus.autogrind
           if(virus.autogrind){
               cplace(R.items[5], 0)
               cplace(R.items[5], virus.math.toRad(90))
               cplace(R.items[5], virus.math.toRad(-90))
       ...
       function cplace(id, angle) {
           if(virus.status !== 'Autobreak'){
           let item = l.list[id];
           if(item) {
               let scale = 35 + item.scale;
               if(nt.checkItemLocation(R.x2 + Math.cos(angle) * scale, R.y2 + Math.sin(angle) * scale, item.scale, 0.6, n.id, false)) {
                   place(id, angle);
               }
           }}
       }
       ...
       virus.scan = (req)=>{
           if(req[1] == R.weapons[0] || req[1] == R.items[2]){
               return true;
           }
           return false;
       }
       ...
       function Si(e, t) {
           if((_ = Ii(e))) {
               _ = Ii(e)
               _.heath = t
               if(_.sid == R.sid && virus.status == 'Antiinsta' && R.shameCount < 7){
                   virus.heal(100-R.health)
                   virus.status = 'Await'
               }
               if(nearestEnemy.length && _.sid !== R.sid && !isAlly(_.sid)){
               if(nearestEnemy[0] == _.sid && _.health <= 25 && (_.skinIndex !== ID_SoldierHelmet && _.skinIndex !== ID_EmpHelmet) && R.skinIndex !== ID_TurretGear){
                   virus.status = 'autoturret'
                   Jt(ID_TurretGear, !1)
               }
                   if(R.primary.reload == 1){
               if(_.health <= 60 && _.skinIndex !== ID_SoldierHelmet){
                   if(dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5){
                       virus.status = 'Autohit'
                       Jt(ID_BullHelmet, !1)
                       Jt(0, !0)
                       r.send("5", R.weapons[0], true);
               ...
               }else if(_.health <= 40 && _.skinIndex == ID_SoldierHelmet){
                   if(dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5){
                       virus.status = 'Autohit'
                       Jt(ID_BullHelmet, !1)
                       Jt(0, !0)
               ...
               if(d >= 0) {
                   ...
                   if(_ == R) {
                       healing(Math.abs(d));
                       if(Math.abs(d) == 5 && virus.status == 'bull'){
                           virus.status = 'Await'
               GameLog.log(virus.status, '#777')
                       }
                       if(Math.abs(d) == 5 && virus.status == 'BullConfig'){
                           r.send('ch', 'Bull Config Complete')
                           virus.status = 'Await'
               GameLog.log(virus.status, '#777')
                           virus.bullConfig = true
                           setTimeout(() => {
                               setInterval(() => {
                                   if(R.shameCount !== 0){
                                       virus.status = 'bull'
       ...
       var allEnemy = [], nearestEnemy = [];
       function Ti(e) {
           if (virus.tickMode && R.primary.reload >= 0.9 && dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5) {
           virus.status = 'Bullspam';
           Jt(ID_BullHelmet, !1);
           Jt(0, !0);
           r.send("5", R.weapons[0], true);
           r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
           ...
           if(R && virus.breaker){
           //if(R.secondary.id == ID_GreatHammer && R.weaponIndex !== R.weapons[1]){Sn(R.weapons[1]);}
           if(R.secondary.id == ID_GreatHammer){
               if(R.secondary.reload == 1){
                   if(R.weaponIndex !== R.weapons[1]){Sn(R.weapons[1], true); R.weaponIndex = R.weapons[1]}
                   virus.status = 'Autobreak'
                   if(R.skins[ID_TankGear])Jt(ID_TankGear, !1)
                   r.send("c", 1, pn());
                   r.send("c", 0, pn());
               }
           }else if(R.primary.reload == 1 && R.secondary.id !== ID_GreatHammer){
               virus.status = 'Autobreak'
               if(R.skins[ID_TankGear])Jt(ID_TankGear, !1)
               r.send("c", 1, pn());
               r.send("c", 0, pn());
...
for (let gg = 0; gg < N.length; gg++) {
    const buildingthing = N[gg];
    if (virus.antipingpong) {
        try {
            if (nearestEnemy[0] && buildingthing.active && buildingthing.owner.sid !== R.sid && !window.isAlly(buildingthing.owner.sid)) {
                if (buildingthing.group.name === 'spikes' && Math.hypot(buildingthing.y - R.y2, buildingthing.x - R.x2) <= 90 && (R.health - buildingthing.dmg - (R.skinIndex === ID_SoldierHelmet ? (R.health - buildingthing.dmg) / 4 : 0)) <= 0) {
                    virus.heal(100 - R.health);
                    console.log('AntiSpike');
                }
                else if (buildingthing.name && buildingthing.name.toLowerCase() === 'turret' && Math.hypot(buildingthing.y - R.y2, buildingthing.x - R.x2) < 800) {
                    virus.autoEmp = true;
                    aaa = true;
                }
            }
    ...
    if (buildingthing.trap && Math.hypot(buildingthing.y - R.y2, buildingthing.x - R.x2) <= 80 && !window.isAlly(buildingthing.owner.sid) && buildingthing.owner.sid !== R.sid && buildingthing.active) {
        virus.inTrap = true;
        aa = true;
        if (R.weaponIndex !== R.weapons[1]) {
            Sn(R.weapons[1], true);
            R.weaponIndex = R.weapons[1];
        }
        if (R.secondary.id === ID_GreatHammer) {
            if (R.secondary.reload === 1) {
                if (R.weaponIndex !== R.weapons[1]) {
                ...
                virus.status = 'Autobreak';
                GameLog.log(virus.status, '#777');
                if (R.skins[ID_TankGear]) Jt(ID_TankGear, false);
                r.send("c", 1, Math.atan2((buildingthing.y - R.y2), (buildingthing.x - R.x2)));
                r.send("c", 0, Math.atan2((buildingthing.y - R.y2), (buildingthing.x - R.x2)));
            }
        }
        else if (R.primary.reload === 1 && !R.secondary.id) {
            virus.status = 'Autobreak';
            GameLog.log(virus.status, '#777');
            if (R.skins[ID_TankGear]) Jt(ID_TankGear, false);
            r.send("c", 1, Math.atan2((buildingthing.y - R.y2), (buildingthing.x - R.x2)));
...
virus.autoEmp = aaa;
           /*
if(virus.WR_Insta && nearestEnemy.length !== 0 && _.sid == nearestEnemy[0]){
    be.drawImage(targetImg2, (_.x - f) - 50, (_.y - d) - 50,100,100);
}*/
           if(R.health <= 50 && virus.status == 'Antiinsta'){
               virus.heal(R.health-100)
           }
           if(R.skins[ID_BullHelmet] && !virus.bullConfig && virus.status !== 'BullConfig'){
               virus.status = 'BullConfig'
               GameLog.log(virus.status, '#777')
               r.send('ch', 'BullConfig')
               Jt(ID_BullHelmet, !1)
           }
           tick++;
           allEnemy = [];
           ...
           for (n = 0; n < e.length; )
               (_ = Ii(e[n])) && (_.t1 = void 0 === _.t2 ? t : _.t2,
                                  ...
                                  _.visible = !0,
                                  _.update(),
                                  !(_ == R || _.team && _.team == R.team) && (allEnemy.push(e.slice(n, n + 13)))),
                   n += 13;
           if(allEnemy.length) {
               allEnemy = allEnemy.sort((a,b) => dist(a, R) - dist(b, R));
               nearestEnemy = allEnemy[0];
           }
           if(nearestEnemy.length && virus.status !== 'autoturret' && virus.status !== 'Autohit'){
           let TargetLBozo = Ii(nearestEnemy[0])
               if(dist(nearestEnemy, R) <= 500 && TargetLBozo.health <= 25 && (TargetLBozo.skinIndex !== ID_SoldierHelmet && TargetLBozo.skinIndex !== ID_EmpHelmet) && R.skinIndex !== ID_TurretGear){
                   virus.status = 'autoturret'
                   Jt(ID_TurretGear, !1)
               }
                   if(R.primary.reload == 1){
               if(TargetLBozo.health <= 60 && TargetLBozo.skinIndex !== ID_SoldierHelmet){
                   if(dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5){
                       virus.status = 'Autohit'
                       Jt(ID_BullHelmet, !1)
                       Jt(0, !0)
               ...
               }else if(TargetLBozo.health <= 40 && TargetLBozo.skinIndex == ID_SoldierHelmet){
                   if(dist(nearestEnemy, R) <= l.weapons[R.primary.id].range * 1.5){
                       virus.status = 'Autohit'
                       Jt(ID_BullHelmet, !1)
                       Jt(0, !0)
           ...
           if(virus.WR_Insta){
               if(virus.status == 'Insta1'){
                   virus.status = 'Insta2'
               GameLog.log(virus.status, '#777')
                   virus.WR_Insta = false;
                   //console.log(virus.WR_Insta)
                   Jt(ID_BullHelmet, !1)
                   r.send("5", R.weapons[0], true);
                   r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                   r.send("c", 0, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                   ...
                   setTimeout(() => {
                       if(dist(nearestEnemy, R) <= 135 && R.primary.reload == 1 && R.secondary.reload == 1 && nearestEnemy[9] != (6 || 22)){
                       virus.status = 'Insta1'
               GameLog.log(virus.status, '#777')
                       //console.log(virus.status)
                       Jt(ID_TurretGear, !1)
                       Jt(0, !0)
                       r.send("5", R.weapons[1], true);
                       r.send("c", 1, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                       r.send("c", 0, Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)));
                       }
                   },virus.WRInstaSeekDelay)
               }
           }
               if(allEnemy.length && Math.hypot(nearestEnemy[2]-R.y2, nearestEnemy[1]-R.x2) <= 150){
                   if(virus.tickMode){
                   cplace(R.items[2], Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)))
                       cplace(R.items[4], Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)) + virus.math.toRad(130))
                       cplace(R.items[4], Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)) - virus.math.toRad(130))
                   }else if(!virus.tickMode){
                   cplace(R.items[2], (Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2))) + virus.math.toRad(180))
                   cplace(R.items[4], Math.atan2((nearestEnemy[2] - R.y2), (nearestEnemy[1] - R.x2)))
                   }
               }
           if(virus.status == 'Await' && !virus.autogrind){
               if(true){
                   if(R.primary.reload !== 1 && R.weaponIndex !== R.weapons[0]){
                       r.send("5", R.weapons[0], true);
                       console.log('autoreload to primary')
                   }
                   if(!virus.tickMode && R.primary.reload == 1 && R.weaponIndex !== R.weapons[1] && R.secondary.id == ID_GreatHammer){
                       r.send("5", R.weapons[1], true);
                       console.log('autoreload to secondary')
                   }
                   /*
               if(R.primary.reload == 1 && R.secondary.id && R.weaponIndex !== R.weapons[1]){
                   r.send("5", R.weapons[1], true);
               }else if(R.primary.reload !== 1 && R.weaponIndex !== R.weapons[0]){
                   if(true){
                   r.send("5", R.weapons[0], true);
                   }
               }
               ...
               if(!virus.autoEmp){
               if(R.skinIndex !== ID_SoldierHelmet && R.skins[ID_SoldierHelmet]){
                   Jt(ID_SoldierHelmet, !1)
               }
               }else{
                   if(R.skinIndex !== ID_EmpHelmet && R.skins[ID_EmpHelmet]){
                   Jt(ID_EmpHelmet, !1)
               }
               }
               if(R.tailIndex !== ID_MonkeyTail && R.tails[ID_MonkeyTail]){
                   Jt(ID_MonkeyTail, !0)
               }
           }else{
               if(virus.autogrind && R.skinIndex !== ID_TankGear && R.skins[ID_TankGear]){
                   Jt(ID_TankGear, !1)
               }
               if(virus.status == 'Insta1'){
                   virus.status = 'Insta2'
                   //console.log(virus.status)
               }else if(virus.status == 'Insta2' || virus.status == 'Spiketick'){
                   virus.status = 'Awaiting'
                   //console.log(virus.status)
               }
               if(virus.status == 'Antiinsta'){
                   if(R.health !== 100)virus.heal(100-R.health)
                   virus.status = 'Awaiting'
               }else{
                   if(virus.status !== 'BullConfig' && virus.status !== 'bull'){
...
let totalPing = 0;
  ...
  numPings++;
  totalPing += e;
  window.AveragePing = totalPing / numPings;
           window.pingTime = e,
               Ie.innerText = `Ping: ${e} ms | Average: ${window.AveragePing}`
       }
       function Bi() {
           Ai = Date.now(),
               r.send("pp")
       ...
       window.requestAnimFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function(e) {
           ...
           function e() {
          if (virus.texts.length !== 0) {
  let totval = 0;
  let { 0: x, 1: y } = virus.texts[0];

  for (let i = 0, len = virus.texts.length; i < len; i++) {
    const [, , n] = virus.texts[i];
    totval += Math.abs(n);
  }

  m.showText(x, y, 50, 0.18, 500, totval, "#f22");
  m.showText(x, y, 60, 0.18, 500, totval, "#000");
  virus.texts = [];
}
           B = Date.now(),
               P = B - q,
            ...
            be.fillRect(0, o.mapScale - o.snowBiomeTop - d, oe, ce - (o.mapScale - o.snowBiomeTop - d))),
               ...
               for (be.globalAlpha = 1,
                   ...
                   if ((_ = W[a] || Y[a - W.length]).visible && (10 != _.skinIndex || _ == R || _.team && _.team == R.team)) {
                       var w = (_.team ? "[" + _.team + "] " : "") + (_.name || "") + (_.isPlayer ? " "+_.shameCount : "");
                       if ("" != w) {
               ...
               !function(e) {
                   if (R && R.alive) {
                       ...
                       It && (Ke.fillStyle = "#fc5553",
                           ...
                           virus.rangeL && (Ke.fillStyle = "#fff",
                                  Ke.font = "34px Hammersmith One",
                                  Ke.textBaseline = "middle",
                                  Ke.textAlign = "center",
                                  Ke.fillText(".", virus.rangeL.x / o.mapScale * Ne.width, virus.rangeL.y / o.mapScale * Ne.height))
                   }
               }(P),
   ...
   , function(e, t, n) {
       ...
       setInterval(() => {
           ...
           send: function(e) {
               ...
               let shouldSend = true
               if(this.connected == !0){
                   if(virus.math.firstOne){
                       virus.math.firstOne = false
                   }else if(true){
                   let arg1 = arguments[0], arg2 = arguments[1]
                   if(arguments[0] !== 'sp' && virus.dead)shouldSend = false
                   switch(arguments[0]){
                       case 'sp':
                           virus.dead = false
                           break;
                       case '2':
                           if(virus.status !== 'Await'){
                               return false;
                           }
                           break;
                       case '5':
                           console.log(arguments[1])
                           if(virus.status == 'Spiketick'){
                               shouldSend = virus.scan(arguments)
                           }
                           break;
                       case 'ch':
                           if(arguments[1] == '!toxic'){
                               virus.toxic = !virus.toxic
                           }
                           if(arguments[1] == '!bull'){
                               virus.bullConfig = false
                           }
                           if(arguments[1] == '!ag'){
                               virus.togAutoGrind()
                           }
                           if(arguments[1] == '!ms'){
                               shouldSend = false;
                               virus.sn(`Average Ping: ${window.AveragePing}ms`)
                           }
                           break;
                   }
                   if(arguments[0] == '33')virus.lastMoveDir = arguments[1];
                   var t = Array.prototype.slice.call(arguments, 1)
                   , n = i.encode([e, t]);
                   if(shouldSend)this.socket.send(n)
               }}
           },
   ...
   , function(e, t) {
       e.exports.groups = [{
           ...
           t.projectiles = window.projectiles = [{
               indx: 0,
               layer: 0,
               src: "arrow_1",
               dmg: 25,
       ...
       e.exports = function(e, t, n, l, h, u, f, d, p, g, m, y, k, v) {
           ...
           this.points = 0,
               ...
               this.active = !0,
                   ...
                   this.weaponXP = [],
                   this.reloads = {},
                   this.primary = {reload: 1, id: 0, variant: 0, dmg: 25, rel: 1},
                   this.secondary = {reload: 1, id: null, variant: 0, dmg: 50, rel: 1},
                   this.turret = 1
           }
               ,
                   ...
                   if(this.buildIndex == -1) {
                       if(this.weaponIndex < 9) {
                           if(this.primary.id == this.weaponIndex) {
                               ...
                               if(this.buildIndex == -1) {
                                   this.primary.rel = this.primary.reload;
                                   this.secondary.rel = this.secondary.reload;
                                   ...
                                   if(this.primary.reload == 1 && this.primary.rel !== 1){
                                       virus.reloadHandle(this)
                                   }
                               }else {
                                   this.primary.rel = this.primary.reload;
                               }
                               ...
                               if(this.primary.id == 4) {
                                   this.secondary.id = 10;
                                   this.secondary.dmg = 10;
                               }else {
